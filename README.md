...
`<logging.dir>/sessions/`. Each file name starts with the issue identifier and
ends with a random session file id so issue debug output can survive daemon
restarts while separate rework contexts stay split.
