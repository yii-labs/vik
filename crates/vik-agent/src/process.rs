...
impl JsonlRpcProcess {
    pub(crate) async fn spawn(command: &str, cwd: &Path) -> Result<Self, AgentError> {
        ...
        let response = self
            .request(
                ...
                json!({
                    "threadId": thread_id,
                    "cwd": cwd,
                    "sandboxPolicy": config.turn_sandbox_policy,
                    "input": [
    ...
    async fn respond_to_server_request(&mut self, message: &Value) -> Result<(), AgentError> {
        ...
        let result = match method {
            ...
            "item/permissions/requestApproval" => json!({ "decision": "acceptForSession" }),
            "item/tool/call" => json!({
                "success": false,
                "contentItems": [
                    { "type": "inputText", "text": format!("unsupported tool call: {method}") }
                ]
            }),
            _ => json!({}),
        };
