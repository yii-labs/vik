...
fn accepts_feishu_tracker_config() {
    ...
    assert_eq!(config.tracker.kind_name(), "feishu");
    let provider = config.tracker.feishu_provider().unwrap();
    assert_eq!(provider.base_token, "P5wZbJ2OiaETjdseIUJczdXqnle");
    assert_eq!(provider.table_id, "tblUqPdAnvAcPY6T");
...
#[test]
fn accepts_feishu_tracker_view_id() {
    let def = parse_workflow_content(
        PathBuf::from("WORKFLOW.md"),
        "---\ntracker:\n  kind: feishu\n  base_token: P5wZbJ2OiaETjdseIUJczdXqnle\n  table_id: tblUqPdAnvAcPY6T\n  view_id: vewpBV8AK0\n---\nBody",
    )
    ...
    assert_eq!(
        config.tracker.feishu_provider().unwrap().view_id,
        "vewpBV8AK0"
    );
    config.validate_for_dispatch().unwrap();
}
...
#[test]
fn feishu_tracker_rejects_invalid_identity() {
    let def = parse_workflow_content(
        PathBuf::from("WORKFLOW.md"),
        "---\ntracker:\n  kind: feishu\n  base_token: P5wZbJ2OiaETjdseIUJczdXqnle\n  table_id: tblUqPdAnvAcPY6T\n  identity: service\n---\nBody",
    )
    ...
    assert!(matches!(
        err,
        WorkflowError::InvalidTrackerCliIdentity(identity) if identity == "service"
    ));
}

#[test]
fn feishu_tracker_requires_base_token() {
    let def = parse_workflow_content(
        PathBuf::from("WORKFLOW.md"),
        "---\ntracker:\n  kind: feishu\n  table_id: tblUqPdAnvAcPY6T\n---\nBody",
    )
    ...
    assert!(matches!(err, WorkflowError::MissingTrackerBaseToken));
}

#[test]
fn feishu_tracker_requires_table_id() {
    let def = parse_workflow_content(
        PathBuf::from("WORKFLOW.md"),
        "---\ntracker:\n  kind: feishu\n  base_token: P5wZbJ2OiaETjdseIUJczdXqnle\n---\nBody",
    )
    ...
    assert!(matches!(err, WorkflowError::MissingTrackerTableId));
}
