...
Current agent subprocess cwd is inherited from the Vik process. Prompts should
tell agents to `cd {{ cwd }}` when issue-workspace cwd matters.
...
- `<workflow-path-key>` is the absolute workflow file path with `/` replaced by
  `-`.
- The workflow-scoped workspace root is auto-created if its parent exists.
...
- `after_create` template context: `issue`, `env`.
- Stage hook context: `cwd`, `workspace`, `issue`, `stage`, `env`.
