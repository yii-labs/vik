...
For local validation, `vik doctor` checks config shape and reports environment
readiness warnings without calling the tracker:
...
perl -0pi -e "s|root: .*|root: ~/code/vik-workspaces-local|" \
  .tests/WORKFLOW.local.md
cargo run --locked -p vik-cli -- doctor .tests/WORKFLOW.local.md
