...
name: linear
description: |
  Use Vik's `linear_graphql` client tool for raw Linear GraphQL
  operations such as comment editing and upload flows.
---

# Linear GraphQL

Use this skill for raw Linear GraphQL work during Vik app-server sessions.

## Primary tool

Use the `linear_graphql` client tool exposed by Vik's app-server session.
It reuses Vik's configured Linear auth for the session.
